#!/usr/bin/env python3
"""Run improved evaluation on test audio set.
...
def print_detailed_log(test_id: str, run: int, total_runs: int, result: dict) -> None:
    ...
    print(f"    transition: {result['transition_used']}")
    ...
    fc = result.get("latency_first_content")
    if fc is not None and fc != "":
        print(f"    first_content: {float(fc):.1f}ms")
...
def main():
    ...
    output_path = output_dir / "improved_results.csv"
    ...
    fieldnames = [
        ...
        "safety_result", "transition_used",
        "latency_asr_latency", "latency_safety_latency",
        "latency_llm_first_sentence", "latency_tts_first_chunk",
        "latency_first_playable", "latency_first_content",
        "latency_total_response", "latency_wall_total",
    ]
        ...
        for run in missing:
            try:
                ...
                row = {
                    ...
                    "transition_used": result.transition_used,
                    ...
                    "latency_first_content": round(summary.get("first_content", 0), 2) if "first_content" in summary else "",
